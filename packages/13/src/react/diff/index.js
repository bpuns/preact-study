...
import { EMPTY_OBJ, options } from '../constants'
...
export function diff(
  ...
  // 函数组件
  out: if (typeof newType == 'function') {
    ...
    // 如果不需要更新，直接return
    if (!shouldUpdate) break out

    options._render && options._render(newVNode)
    ...
    options._diffed && options._diffed(c)
...
export function commitRoot(commitQueue) {
  ...
  options._commit && options._commit(commitQueue)
